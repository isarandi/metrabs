...
class Model25D(keras.Model):
    def __init__(self, backbone, joint_info, bone_lengths_ideal):
        ...
        if FLAGS.transform_coords:
            self.recombination_weights = tf.constant(np.load('32_to_122.npy'))
        if 'many' in FLAGS.dataset:
            selected_bones = [
                i for i, (j1, j2) in enumerate(joint_info.stick_figure_edges)
                if (joint_info.names[j1].endswith('_muco') and
                    joint_info.names[j2].endswith('_muco'))]
            self.edges = list(np.array(self.edges)[selected_bones])
            self.bone_lengths_ideal = list(np.array(self.bone_lengths_ideal)[selected_bones])
...
class Head25D(keras.layers.Layer):
    ...
    def call(self, inp, training=None):
        ...
        return models.util.heatmap_to_25d(coords_heatmap, training)


class Model25DTrainer(models.model_trainer.ModelTrainer):
    def __init__(self, model25d, joint_info, joint_info2d=None, global_step=None):
    ...
    def forward_train(self, inps, training):
        ...
        preds.coords3d_pred_abs = tfu3d.reconstruct_absolute_by_bone_lengths(
            preds.coords25d_pred, inps.intrinsics, self.model.bone_lengths_ideal, self.model.edges)
