# Code modified by Istvan Sarandi, based on TensorFlow sources.
# Copyright and license notice from the original TensorFlow authors follows below:
#
    ...
    with variable_scope.variable_scope(
        ...
        with arg_scope(
            ...
            with arg_scope([layers.batch_norm], is_training=is_training):
                ...
                # Appendix of [2].
                nets = [layers.batch_norm(
                    net, activation_fn=nn_ops.relu, scope=suffixed_name('postnorm', i_branch))
                    for i_branch, num_classes_ in enumerate(num_classes)]

                if global_pool:
                    nets = [tf.reduce_mean(net, tfu.image_axes(), keepdims=True) for net in nets]

                if num_classes is not None:
                    nets = [layers_lib.conv2d(
                        net, num_classes_, [1, 1], activation_fn=None, normalizer_fn=None,
                        scope=suffixed_name('logits', i_branch))
...
def suffixed_name(name, number):
    if FLAGS.compatibility_mode:
        return name
    else:
        return f'{name}_copy{number}'
...
def resnet_v2_50(
        ...
        centered_stride=False, reuse=None, scope='resnet_v2_50'):
    ...
    if centered_stride:
        ...
        if i_last >= 0:
            c[i_last] = True
    blocks = [
        resnet_v2_block('block1', base_depth=64, num_units=3, stride=2, centered_stride=c[0]),
        resnet_v2_block('block2', base_depth=128, num_units=4, stride=2, centered_stride=c[1]),
        resnet_v2_block('block3', base_depth=256, num_units=6, stride=2, centered_stride=c[2]),
...
def resnet_v2_101(
        inputs, num_classes=None, is_training=True, global_pool=True, output_stride=None,
        centered_stride=False, reuse=None, scope='resnet_v2_101'):
    """ResNet-101 model of [1]. See resnet_v2() for arg and return description."""

    c = [False, False, False]
    if centered_stride:
        i_last = int(np.log2(output_stride)) - 3
        c[i_last] = True

    blocks = [
        resnet_v2_block('block1', base_depth=64, num_units=3, stride=2, centered_stride=c[0]),
        resnet_v2_block('block2', base_depth=128, num_units=4, stride=2, centered_stride=c[1]),
        resnet_v2_block('block3', base_depth=256, num_units=23, stride=2, centered_stride=c[2]),
        resnet_v2_block('block4', base_depth=512, num_units=3, stride=1),
    ]
    return resnet_v2(
        inputs, blocks, num_classes, is_training, global_pool, output_stride,
...
def resnet_v2_152(
        inputs, num_classes=None, is_training=True, global_pool=True, output_stride=None,
        centered_stride=False, reuse=None, scope='resnet_v2_152'):
    ...
    blocks = [
        resnet_v2_block('block1', base_depth=64, num_units=3, stride=2, centered_stride=c[0]),
        resnet_v2_block('block2', base_depth=128, num_units=8, stride=2, centered_stride=c[1]),
        resnet_v2_block('block3', base_depth=256, num_units=36, stride=2, centered_stride=c[2]),
        resnet_v2_block('block4', base_depth=512, num_units=3, stride=1),
    ]
    return resnet_v2(
        inputs, blocks, num_classes, is_training, global_pool, output_stride,
...
def resnet_v2_200(
        inputs, num_classes=None, is_training=True, global_pool=True, output_stride=None,
        centered_stride=False, reuse=None, scope='resnet_v2_200'):
    ...
    blocks = [
        resnet_v2_block('block1', base_depth=64, num_units=3, stride=2, centered_stride=c[0]),
        resnet_v2_block('block2', base_depth=128, num_units=24, stride=2, centered_stride=c[1]),
        resnet_v2_block('block3', base_depth=256, num_units=36, stride=2, centered_stride=c[2]),
        resnet_v2_block('block4', base_depth=512, num_units=3, stride=1),
    ]
    return resnet_v2(
